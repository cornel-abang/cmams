@extends('layouts.dashboard')
@section('content')
{{-- @section('sweet-alert-area')
    <script src="{{asset('assets/js/sweetalert2.js')}}" defer></script>
@endsection --}}

<!-- [ stiped-table ] start -->
            <div class="col-xl-12">
                <div class="card">
                    <div class="card-header">
                        <h5>{{$title}}</h5>
                        <span class="d-block m-t-5">
                            A total of <b><code>{{$misseds->count()}}</code></b> clients have missed their appointments so far.
                        </span>
                        <button type="button" class="btn btn-info btn-sm add-btn">
                            <a href="{{route('export-missed')}}" style="color: white; font-weight: bold;">
                                <i class="la la-download"></i> Export Today
                            </a>
                        </button>
                    </div>
                    <div class="card-body table-border-style">
                        <div class="table-responsive">
                        
                            <table class="table table-striped" id="entry-table">
                                <thead>
                                    <tr>
                                        <th>Client</th>
                                        <th>Case Manager Involved</th>
                                        <th>Due Date</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach($misseds as $miss)
                                    <tr>
                                        <td>{{ $miss->client }}</td>
                                        <td>{{ $miss->case_manager }}</td>
                                        <td>{{ Carbon\Carbon::parse($miss->appt_date)->format('l jS \of F Y') }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                            {{-- {{ $appointments->links() }} --}}
                        </div>
                    </div>
                </div>
            </div>
    </div>

    @endsection
