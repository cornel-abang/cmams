(function($) {
    ...
    form.steps({
        ...
        {
            let facility = $('select[name=sel-facility] option').filter(':selected').val();
            if (!facility) {
                alert("Please select facility");
            }else{
                $('#daily_rep_frm').append(`<input type="text" name="facility" value="${facility}" class="hidden-facility">`);
                $('#daily_rep_frm').submit();
            }
        },
